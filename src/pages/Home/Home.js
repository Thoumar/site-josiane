...
const Home = ({ homeData, projects, peoples }) => {
      ...
      if (target === "work") {
        ...
        setTimeout(function () {
          if (document.querySelector('[scroll-ref="' + target + '"]')) {
            document
              .querySelector('[scroll-ref="' + target + '"]')
              .scrollIntoView({
                behavior: "smooth",
                block: "start",
                inline: "center",
  ...
  const logoScrollListenerHandler = (e) => {
    ...
    if (
      window.pageYOffset > logoCousinesRef.current.offsetTop &&
      window.pageYOffset < logoCousinesRef.current.offsetTop + 700
    ) {
    ...
    return (
      <main className="Home">
        ...
        <Button
          ...
          click={() => setShowIsotope(!showIsotope)}
        />
        ...
        >
          <img
            className="Logo__Picture Logo__Et"
            src={logoEtCousines}
            onClick={() => history.push("/")}
            alt="Et Logo"
          />
